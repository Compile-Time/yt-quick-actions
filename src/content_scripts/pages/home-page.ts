...
function setupWatchLaterButtonIfNotPresent(videoMenuButton: HTMLElement): void {
  ...
  if (
    ...
    divDismissible.appendChild(
      createWatchLaterButton(videoMenuButton).completeHtmlElement
    );
  }
}

function createWatchLaterButton(
  videoMenuButton: HTMLElement
): QaButtonInContainer {
  const watchLaterButton = QaHtmlElements.watchLaterHomeVideoButton();
  watchLaterButton.buttonElement.onclick = () => {
    contentScriptObserversManager
      .upsertOneshotObserver(saveToWatchLaterEntryObserver)
      .observe();
          ...
          summaries[0].removed
            .map(
              (ytdMenuServiceItemRenderer) =>
                ytdMenuServiceItemRenderer as HTMLElement
            )
            .filter((ytdMenuServiceItemRenderer) =>
              HtmlTreeNavigator.startFrom(ytdMenuServiceItemRenderer)
                .findFirst(
                  new SvgDrawPathNavigationFilter(SVG_DRAW_PATH.WATCH_LATER)
                )
                .exists()
            )
 ...
 */
function initHomePageVideosLoadingObserverNew(
  divContainerForYtdRichGridRows: Node
): void {
        ...
        if (summaries[0].added.length > 0) {
          summaries[0].added
            ...
            .forEach((moreOptionsButton) =>
              setupWatchLaterButtonIfNotPresent(moreOptionsButton)
            );
        }
      },
...
function initContentScript(ytdRichGridRows: HTMLElement[]): void {
  ...
  contentScriptObserversManager
    .addBackgroundObserver(homePageVideosLoadingObserver)
    .observe();
}
  ...
  MutationElementExistsWatcher.build()
    .queryFn(() => {
      const ytdRichGridRows = HtmlTreeNavigator.startFrom(document.body)
        ...
        .filter(
          new TagNavigationFilter(Tags.YTD_TWO_COLUMN_BROWSE_RESULTS_RENDERER)
        )
    ...
    .then((elementWatcherResult) => {
      ...
      const ytdRichGridRows =
        elementWatcherResult.ytdRichGridRows as HTMLElement[];
